<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="养老金发放" prop="yanglaojinfafang">
          <el-select v-model="ruleForm.yanglaojinfafang" placeholder="请选择养老金发放">
            <el-option
                v-for="(item,index) in yanglaojinfafangOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="养老金发放" prop="yanglaojinfafang">
	      <el-input v-model="ruleForm.yanglaojinfafang"
                placeholder="养老金发放" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="身体状况" prop="shentizhuangkuang">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="身体状况"
                  v-model="ruleForm.shentizhuangkuang" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shentizhuangkuang" label="身体状况" prop="shentizhuangkuang">
                    <span>{{ruleForm.shentizhuangkuang}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shentizhuangkuang = true;
          ...
          if(o=='yanglaojinfafang'){
            this.ruleForm.yanglaojinfafang = obj[o];
	    this.ro.yanglaojinfafang = true;
            continue;
          }
          if(o=='zhaopian'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `aixinzhulao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.aixinzhulaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
